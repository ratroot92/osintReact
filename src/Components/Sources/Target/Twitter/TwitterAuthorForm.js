...
fetch('http://192.168.18.56:8000/core/add_twitter_target/' , {
 ...
 cache: "no-cache",
  ...
  need_screenshots:this.state.need_screenshots,
   periodic_interval:this.state.periodic_interval,
  tweets_type:this.state.tweets_types,}),
})
...
populatetweetetype(){
  ...
  .then(response=> {
   this.setState(
        {tweets_type:response.data.twitter_target_form.tweets_type}
        )
    console.log("feteched tweets_type");
  })
    ...
    return (<form method="POST" id='myform' onSubmit={this.handleSubmit}>
      ...
      <div className="form-group form-inline mt-5 mb-5">
          <label className="col-4 font-weight-bold  text-primary" for="">select Interval</label>
            ...
            <option value="0">Select interval </option>{this.state.intervals.map(interval =><option value={interval[0]}>{interval[1]}</option>)}
            </select>
          </div>
      ...
      <div className="form-group form-inline mt-5 mb-5">
          <label className="col-4 font-weight-bold  text-primary" for="">select Interval</label>
            <select className="form-control col-5" name="tweets_type"  onChange={this.handleInputChange} required="required">
            <option value="0">Select tweets type </option>{this.state.tweets_type.map(type =><option value={type[0]}>{type[1]}</option>)}
            </select>
          </div>
